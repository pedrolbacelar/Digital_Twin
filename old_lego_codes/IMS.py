...
def on_connect(client, userdata, flags, rc):
  ...
  client.subscribe("topic/control-download")
...
#    op_pos[ op['station'] -1 ] = op['operator']
  ...
  if msg.topic == "topic/control-production":
    
    cp = json.loads(msg.payload.decode())
    production_allowed[ cp['station'] - 1 ] = cp['control']
    #print(production_allowed)
    
  if msg.topic == "topic/control-download":
    
    da = json.loads(msg.payload.decode())
    download_allowed[ da['station'] -1 ] = da['control']
    #print(download_allowed)
...
try:
  ...
  while cond_while:  # NOTE, can also be interrupted by CTRL+C (raises KeiboardInterrupt)
    ...
    blocking_opticalsensor.mode = 'COL-REFLECT'
    ...
    # CONTROL VARIABLES INIT (ALL PRODUCTION IS ALLOWED)
    production_allowed = [1, 1, 1, 1, 1, 1] # production is allowed in all stations
    download_allowed = [1, 1, 1, 1, 1, 1] # piece download is allowed in all stations
    ...
    try:
      while cond_while:
          ...
          ##############
          # DEBUG
          #print("PRODUCTION ALLOWED"+str(production_allowed[station-1]))
          #print("DOWNLOAD ALLOWED"+str(download_allowed[station-1]))
          ##############
          
          if production_allowed[station] == False:
              print("production NOT ALLOWED")
              sleep(1)
          
          if color_stat_available < threshold_low  and color_conv_entering > threshold_high and production_allowed[station-1]:
              ...
              if not Timeout:
                  ...
                  if w_distribution_type == 'triangular' or w_distribution_type == 'Triangular':
                    ...
                    Tw_Astoc = random.normalvariate(w_distribution_par[0], w_distribution_par[1]*(1+counter*deg_param)) # DEGRADATION HERE
                  ...
                  if random.random() <= (1- failure_prob):   # WORKING STATE
                    ...
                    #publish the control action - production is NOT ALLOWED on other machines
                    client.publish("topic/control", 0)
                    ...
                    # joint maintenance start
                    if len(joint_maintenance)>0:
                      for s in joint_maintenance:
                      
                        a = {"station": s, "control": 0}
                        data_out = json.dumps(a, indent = 4)
                        client.publish("topic/control-production", data_out)
                    ...
                    workingA = max(Tw_Astoc,Tw_Amin) + max(Tf_Amin, Tf_Astoc)
                    ...
                    counter = 0 # RE-ESTABLISH THE DEGRADATION COUNTER TO 0, SO THE STD OF PRODUCTION TIME RETURNS TO THE ORIGINAL VALUE
                    
#                    #publish the control action - production is back to ALLOWED on other machines
#                    client.publish("topic/control", 1)
                    
                    # joint maintenance stop -- allow production in the joint maintenance stations
                    if len(joint_maintenance)>0:
                      for s in joint_maintenance:
                      
                        a = {"station": s, "control": 1}
                        data_out = json.dumps(a, indent = 4)
                        client.publish("topic/control-production", data_out)
                  ...
                  while download_allowed[station-1] == False:
                    sleep(1)
                    print("DOWNLOAD IS NOT ALLOWED")
                    
                  # Verify if the Bocking Condition happens
                  #color_block = colors[blocking_opticalsensor.value()]
                  
                  if blocking_opticalsensor.value() >= threshold_high:
                      Blocked = True
                      set = False
                      #last_seen = []
