...
class Exit(CoreObject):
    ...
    def __init__(self, id, name, cancelCondition={}, gatherSysTimeEx=True, gatherIntDepEx=True, **kw):
        ...
        self.Lifespan = []
        self.TaktTime = []
        self.SysTime_E = []       # initialize the systime for multiple runs
        self.IntDep_E = []
        ...
        self.gatherSysTimeEx = gatherSysTimeEx          # Change the value of gatherSysTimeEx to True if specified in the input file (model)
        self.gatherIntDepEx = gatherIntDepEx
    ...
    def getEntity(self):
        ...
        self.actual_lifespan = self.env.now - activeEntity.startTime    # lifespan of the activeEntity
        self.numOfExits += 1  # increase the exits by one
        # concatenate to the systime list an array with the number of the part and its lifespan
        # if self.numOfExits == 1:
        #     self.SysTimeList_E = np.array([[1, self.actual_lifespan]])  # initialize the array for the systime trace
        if self.numOfExits == 1:
            self.SysTimeList_E = np.array([[self.env.now, self.numOfExits, self.actual_lifespan]])  # initialize the array for the systime trace
        # else:
        #     self.SysTimeList_E = numpy.concatenate(
        #         (self.SysTimeList_E, [[self.numOfExits, self.actual_lifespan]])
        #     )
        else:
            self.SysTimeList_E = numpy.concatenate(
                (self.SysTimeList_E, [[self.env.now, self.numOfExits, self.actual_lifespan]])
            )
        if self.numOfExits == 1:
            pass
        elif self.numOfExits == 2:
            self.IntDepList_E = np.array([[self.env.now, self.numOfExits, self.env.now - self.timeLastEntityLeft]])
        else:
            self.IntDepList_E = np.concatenate(
                (self.IntDepList_E, [[self.env.now, self.numOfExits, self.env.now - self.timeLastEntityLeft]])
            )
    ...
    def postProcessing(self, MaxSimtime=None, correction=None):
        ...
        self.SysTime_E.append(self.SysTimeList_E.tolist())
        self.IntDep_E.append(self.IntDepList_E.tolist())
    ...
    def outputResultsJSON(self):
        ...
        if self.gatherSysTimeEx:      # output the systime trace as result only if specified
            json["results"]["systime_list"] = self.SysTime_E
        if self.gatherIntDepEx:
            json["results"]["intdep_list"] = self.IntDep_E
        if (
