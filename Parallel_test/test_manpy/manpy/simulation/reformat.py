...
def positionGraph(g):
    ...
    origin_left, origin_top, max_left, max_top = [
        float(p) for p in new_graph.get_bb()[1:-1].split(",")
    ]
    ...
    for node in new_graph.get_nodes():
        ...
        preference_dict[get_name(node)] = dict(
            top=1 - (top / ratio_top), left=1 - (left / ratio_left),
        )
...
def format(m):
    for node in list(m["nodes"].values()):
        ...
        if "processingTime" in node:
            ...
            if "entity" in node:
                entity = node["entity"]
                if not entity.startswith("manpy."):
                    node["entity"] = "manpy.%s" % entity
        if "wip" in node:
            for job in node["wip"]:
                if "route" in job:
                    for r in job["route"]:
