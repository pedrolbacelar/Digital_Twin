...
from .ProcessingTimeList import ProcessingTimeList
...
class Machine(CoreObject):
    ...
    def __init__(
        ...
        technology=None,
        priority=0,
        gatherSysTime=False,                # flag if we want to calculate the system time from the entrance on this machine
        gatherIntArr=False,                 # flag if we want to calculate the interarrival time trace at the start of the processing
        freq_parallel=1,
        **kw,
    ):
        ...
        if list(processingTime.keys())[0] == "fromList":                   # Check if the processing times are from list
            self.fromListFlag = True                        # flag if processing times are from list
        else:
            processingTime = self.getOperationTime(time=processingTime)
            self.fromListFlag = False

        setupTime = self.getOperationTime(time=setupTime)

        loadTime = self.getOperationTime(time=loadTime)

        unloadTime = self.getOperationTime(time=unloadTime)

        #     holds the capacity of the machine
        self.capacity = capacity
        self.gatherSysTime = gatherSysTime
        self.gatherIntArr = gatherIntArr
        ...
        if self.fromListFlag:
            self.procTimeVal = ProcessingTimeList(processingTime["fromList"]["path"], None)
        else:
        ...
        # define the unload times
        self.unloadRng = RandomNumberGenerator(self, unloadTime)
        ...
        self.freq_parallel = freq_parallel
    ...
    def __repr__(self):
        if self.fromListFlag:
            return f"Machine({self.id!r}, {self.name!r}, processingTime='fromList', " \
                   f"gatherSysTime={bool(self.gatherSysTime)}, gatherIntArr={bool(self.gatherIntArr)})"
        else:
            return f"Machine({self.id!r}, {self.name!r}, processingTime={self.rng!r}, " \
                   f"gatherSysTime={bool(self.gatherSysTime)}, gatherIntArr={bool(self.gatherIntArr)})"
    ...
    def endOperationActions(self, type):
        ...
        if type == "Processing":
            ...
            # Append to the eventlist the time the operation has finished
            if G.calculateEventList and self.name == "Machine":
                G.MachineEventList[0].append(round(self.env.now, 2))  # append the time
                G.MachineEventList[1].append(self.id[-1])  # append the activity (which machine)
                G.MachineEventList[2].append(self.completedJobs)  # append the part ID
                G.MachineEventList[3].append('f')  # append the action
    ...
    def getEntity(self):
        ...
        # after the machine receives an entity, we must save the time in the eventlog
        if G.calculateEventList and self.name == "Machine":
            G.MachineEventList[0].append(round(self.env.now, 2))      # append the time
            G.MachineEventList[1].append(self.id[-1])      # append the activity (which machine)
            G.MachineEventList[2].append(self.completedJobs+1)  # append the part ID
            G.MachineEventList[3].append('s')       # append the action
    ...
    def outputResultsJSON(self):
        ...
        json["results"]["completed_jobs"] = self.jobsDone          # I added completed jobs to the result in the json file
        if self.gatherSysTime:
            json["results"]["system_time_trace"] = self.SysTime     # prints the systime trace only if flagged
        if self.gatherIntArr:
            json["results"]["interarrival_trace"] = self.IntArr     # prints the interarrival trace only if flagged
