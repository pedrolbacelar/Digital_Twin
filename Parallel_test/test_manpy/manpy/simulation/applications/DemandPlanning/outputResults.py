...
def outputResults():
    ...
    resultsOverview.append(
        ["selected scenario", G.Summary["bestScenario"]]
    ...
    for bottleneck in G.Bottlenecks:
        initialCap = [
            G.Capacity[bottleneck][week]["OriginalCapacity"] for week in G.WeekList
        ]
        G.CapacityResults.append(
            [bottleneck, "Capa Pegging Resource Capacity (UoM)",] + initialCap
        )
        G.CapacityResults.append(
            ["", "Capa Pegging Resource Total Load (UoM)",]
            + [
                G.Capacity[bottleneck][week]["OriginalCapacity"]
                - G.CurrentCapacityDict[bottleneck][week]
        ...
        G.CapacityResults.append(
            ["", "Capa Pegging Resource Min Util (Percent)",]
            + [G.Capacity[bottleneck][week]["minUtOrig"] * 100 for week in G.WeekList]
        )
        G.CapacityResults.append(
            ["", "Capa Pegging Resource Target Util (Percent)",]
            + [
                G.Capacity[bottleneck][week]["targetUtilisation"] * 100
                for week in G.WeekList
            ]
        )
    ...
    for bottleneck in G.Bottlenecks:
        ...
        for week in G.WeekList:
            ...
            G.Utilisation[bottleneck][week]["averageUtilization"] = (
                float(
                    G.Capacity[bottleneck][week]["OriginalCapacity"]
                    - G.CurrentCapacityDict[bottleneck][week]
                )
            ...
            G.Utilisation[bottleneck][week]["minUtilization"] = G.Capacity[bottleneck][
                week
            ]["minUtilisation"]
            G.Utilisation[bottleneck][week]["maxUtilization"] = G.Capacity[bottleneck][
                week
            ]["targetUtilisation"]
    ...
    for sp in G.SPs:
        ...
        for ma in G.SPlist[sp]:
                ...
                for week in G.WeekList:
                    ...
                    if week in G.sortedOrders["order"][priority]:
                        for i in range(len(G.sortedOrders["order"][priority][week])):
                            if (
                                G.orders[
                                    G.sortedOrders["order"][priority][week][i][
                                        "orderID"
                                    ]
                                ]["chosenMA"]
                                == ma
                            ):
                                temp += G.sortedOrders["order"][priority][week][i][
                                    "Qty"
                                ]
                    ...
                    for bot in G.RouteDict[ma]:
                        G.Butilisation[bot][ma][week] += (
                            G.RouteDict[ma][bot][week]
                            * G.globalMAAllocation[ma][week]["order"][priority]
                            / G.Capacity[bot][week]["OriginalCapacity"]
                            * 100
                        )

                if newSp != None:
                    G.allocationResults.append(
                        ["", sp, ma, "ORDERS", priority, "Demand Request Qty"] + orderMA
                    )
                    newSp = None
                else:
                    G.allocationResults.append(
                        ["", "", ma, "ORDERS", priority, "Demand Request Qty"] + orderMA
                    )
                G.allocationResults.append(
                    ["", "", "", "ORDERS", priority, "Demand Planned Qty"]
                    + [
                        G.globalMAAllocation[ma][week]["order"][priority]
                        for week in G.WeekList
                    ]
            ...
            if len(G.priorityList["order"]) == 0:
                if newSp != None:
                    G.allocationResults.append(
                        ["", sp, ma, "ORDERS", 0, "Demand Request Qty"]
                        + [0 for i in range(len(G.WeekList))]
                    )
                    newSp = None
                else:
                    G.allocationResults.append(
                        ["", "", ma, "ORDERS", 0, "Demand Request Qty"]
                        + [0 for i in range(len(G.WeekList))]
                    )
                G.allocationResults.append(
                    ["", "", "", "ORDERS", 0, "Demand Planned Qty"]
                    + [0 for i in range(len(G.WeekList))]
                )
            ...
            if len(G.priorityList["forecast"]) == 0:
                G.allocationResults.append(
                    ["", "", "", "FORECAST", 0, "Demand Request Qty"]
                    + [0 for i in range(len(G.WeekList))]
                )
                G.allocationResults.append(
                    ["", "", "", "FORECAST", 0, "Demand Planned Qty"]
                    + [0 for i in range(len(G.WeekList))]
                )
            ...
            for priority in G.priorityList["forecast"]:
                    ...
                    if week in G.sortedOrders["forecast"][priority]:
                        for i in range(len(G.sortedOrders["forecast"][priority][week])):
                            if (
                                ma
                                in G.orders[
                                    G.sortedOrders["forecast"][priority][week][i][
                                        "orderID"
                                    ]
                                ]["suggestedMA"]
                            ):
                            ...
                            if (
                                ...
                                == sp
                                and G.sortedOrders["forecast"][priority][week][i][
                                    "orderID"
                                ]
                                not in countedForecastSP[week]
                            ):
                                spReqQty[sp][week] += G.orders[
                                    G.sortedOrders["forecast"][priority][week][i][
                                        "orderID"
                                    ]
                                ]["Qty"]
                                countedForecastSP[week].append(
                                    G.sortedOrders["forecast"][priority][week][i][
                                        "orderID"
                                    ]
                                )
                ...
                G.allocationResults.append(
                    ["", "", "", "FORECAST", priority, "Demand Request Qty"] + orderMA
                )
                G.allocationResults.append(
                    ["", "", "", "FORECAST", priority, "Demand Planned Qty"]
                    + [
                        G.globalMAAllocation[ma][week]["forecast"][priority]
                        for week in G.WeekList
                    ]
                )
            ...
            for week in G.WeekList:
                ...
                for orderType in ["order", "forecast"]:
                    for priority in G.priorityList[orderType]:
                        maPlannedQty[week] += G.globalMAAllocation[ma][week][orderType][
                            priority
                        ]
            ...
            G.allocationResults.append(
                ["", "", ma + "Demand Request Qty", "", "", ""] + orderMA
            )
            G.allocationResults.append(
                ["", "", ma + "Demand Planned Qty", "", "", ""] + plannedMA
            )
        ...
        G.allocationResults.append(
            ["", sp + "Demand Request Qty", "", "", "", ""] + orderSP
        )
        G.allocationResults.append(
            ["", sp + "Demand Planned Qty", "", "", "", ""] + plannedSP
        )
        ...
        for sp in list(G.SPlist.keys()):
            ...
            for typeOrder in ["order", "forecast"]:
                for priority in G.priorityList[typeOrder]:
                    qtyRif[week][sp]["tot"] += G.globalMAAllocationIW[sp][week][
                        typeOrder
                    ][priority]
            ...
            for ma in G.SPlist[sp]:
                ...
                if len(G.Lateness[week][ma]["qty"]):
                    G.Lateness[week][ma]["result"] = (
                        sum(G.Lateness[week][ma]["lateness"]) * 100
                    )
                else:
                    G.Lateness[week][ma]["result"] = 0
                for typeOrder in ["order", "forecast"]:
                    for priority in G.priorityList[typeOrder]:
                        qtyRif[week][sp]["tot"] += G.globalMAAllocationIW[ma][week][
                            typeOrder
                        ][priority]
                        qtyRif[week][sp][ma] += G.globalMAAllocationIW[ma][week][
                            typeOrder
                        ][priority]

            for ma in G.SPlist[sp]:
                if qtyRif[week][sp]["tot"]:
                    weightedLateSP[week][sp] += (
                        G.Lateness[week][ma]["result"] * float(qtyRif[week][sp][ma])
                    ) / qtyRif[week][sp]["tot"]

    for sp in G.SPs:
        latenessResults.append([sp] + [weightedLateSP[week][sp] for week in G.WeekList])
        for ma in G.SPlist[sp]:
            latenessResults.append(
                [ma] + [G.Lateness[week][ma]["result"] for week in G.WeekList]
            )
        ...
        for sp in list(G.SPlist.keys()):
            ...
            for ma in G.SPlist[sp]:
                if len(G.Earliness[week][ma]["qty"]):
                    G.Earliness[week][ma]["result"] = (
                        sum(G.Earliness[week][ma]["earliness"]) * 100
                    )
                else:
            ...
            for ma in G.SPlist[sp]:
                if qtyRif[week][sp]["tot"]:
                    weightedEarlySP[week][sp] += (
                        G.Earliness[week][ma]["result"] * float(qtyRif[week][sp][ma])
                    ) / qtyRif[week][sp]["tot"]

    for sp in G.SPs:
        earlinessResults.append(
            [sp] + [weightedEarlySP[week][sp] for week in G.WeekList]
        )
        for ma in G.SPlist[sp]:
            earlinessResults.append(
                [ma] + [G.Earliness[week][ma]["result"] for week in G.WeekList]
            )
    ...
    excessStats.append(
        [
            "Lateness",
            G.LateMeasures["noLateOrders"],
            mean(G.LateMeasures["lateness"]) * 100,
        ]
    )
    excessStats.append(
        [
            "Earliness",
            G.LateMeasures["noEarlyOrders"],
            mean(G.LateMeasures["earliness"]) * 100,
        ]
    )
    excessStats.append(
        ["Excess", G.LateMeasures["noExcess"], G.LateMeasures["exUnits"]]
    )
    ...
    for bottleneck in G.Bottlenecks:
        ...
        for sp in G.SPs:
            for ma in G.SPlist[sp]:
                if new:
                    BottUtilisation.append(
                        [bottleneck, ma]
                        + [G.Butilisation[bottleneck][ma][week] for week in G.WeekList]
                    )
                    new = 0
                else:
                    BottUtilisation.append(
    ...
    with open(
        "Results\\rTable.html", "wb"
    ) as h:  # completion time, cycle time and delay info in html format
