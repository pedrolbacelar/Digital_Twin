...
def utilisationCalc1(ACOcapacityDict, initialWeek, ind):
    ...
    for bottleneck in G.Bottlenecks:
        weekList = [initialWeek] + [
            G.WeekList[i] for i in range(ind - 1, max(-1, ind - G.maxEarliness - 1), -1)
        ]
        minU = []
        targetU = []
        for week in weekList:
            utilisation = (
                float(
                    ...
                    - ACOcapacityDict[bottleneck][week]
                )
                / G.Capacity[bottleneck][week]["OriginalCapacity"]
            )
            ...
            if G.Capacity[bottleneck][week]["targetUtilisation"]:
                targetU.append(
                    float(
                        utilisation - G.Capacity[bottleneck][week]["targetUtilisation"]
                    )
    ...
    ACOtargetUtil = mean(
        array(targetUtil)
    )  # mean(array(targetUtil)) #FIXME: potrebbe essere std(array(targetUtil))
...
def utilisationCalc3(ACOcapacityDict, initialWeek, ind):
    ...
    ACOminUtil = mean(
        array(minUtil)
    )  # FIXME: considerare piu settimane in weeklist e std(targetUtil)
