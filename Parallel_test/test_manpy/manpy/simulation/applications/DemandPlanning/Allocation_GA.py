...
def Allocation_GA(initialWeek, itemList, itemType, GAresults):
    ...
    chromosomes = (
        []
    )  # list of ants that are being evaluated, an ant is a combination of different weighting factors for multi-obj optimisation (PB assignment)
    testedChrom = []
    ...
    while chromoID < G.popSizeGA:
        ...
        # simulate chromosome
        resultGA = AllocationRoutine_ForecastGA(
            initialWeek, orderList, itemType, chromo
        )
    ...
    for gen in range(1, G.noGenGA):
        ...
        for item in range(G.popSizeGA):
            ...
            if item < G.popSizeGA - 1 and xOver <= G.probXover:
                ...
                (
                    chromosomes[item]["chromo"]["seq"],
                    chromosomes[item + 1]["chromo"]["seq"],
                ) = order2x(
                    chromosomes[item]["chromo"]["seq"],
                    chromosomes[item + 1]["chromo"]["seq"],
                )
                changeC[
                    item
                ] = 1  # both chromosomes have changes and they should be reassessed
                changeC[item + 1] = 1
            ...
            if mutation <= G.probMutation:
                ...
                chromosomes[item]["chromo"]["seq"] = displacement(
                    chromosomes[item]["chromo"]["seq"]
                )
                changeC[item] = 1
            ...
            # simulate chromosome
            resultGA = AllocationRoutine_ForecastGA(
                initialWeek, orderList, itemType, chromosomes[item]["chromo"]
            )
            ...
            GAresults.append(
                (
                    ...
                    gen,
                    item,
                    chromosomes[item]["chromo"]["cID"],
                    chromosomes[item]["excess"],
                    chromosomes[item]["lateness"],
                    chromosomes[item]["earliness"],
                    chromosomes[item]["targetUtil"],
                    chromosomes[item]["minUtil"],
                    chromosomes[item]["chromo"]["seq"],
                )
            )
