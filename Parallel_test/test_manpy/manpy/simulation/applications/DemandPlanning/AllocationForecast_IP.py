...
def Allocation_IP(item, week, previousAss, capacity, weightFactor):
    ...
    for bottleneck in G.Bottlenecks:
        ...
        for ma in MAlist:
            if bottleneck in G.RouteDict[ma]:
                requiredCapacity[bottleneck].append(
                    G.RouteDict[ma][bottleneck][week]
                    * MA_var[ma]
                    * G.BatchSize[ma][week]
                )

        capDict_obj[bottleneck] = lpSum(
            [-1 * capacity[bottleneck][week]]
            + [requiredCapacity[bottleneck]]
        ...
        utilisation[bottleneck] = (
            1.0
            / float(G.Capacity[bottleneck][week]["OriginalCapacity"])
            * capDict_obj[bottleneck]
        )
        ...
        prob += (
            lpSum(
                [
                    ...
                    / float(G.Capacity[bottleneck][week]["targetUtilisation"])
                ]
            )
            >= Delta_targetUt[bottleneck]
        )
        prob += (
            lpSum(
                [
    ...
    if weightFactor[2]:
        h1 += [
            Delta_targetUt[bottleneck] * weightFactor[2] for bottleneck in G.Bottlenecks
        ]

    # aggregate objective
    if weightFactor[3]:
        # third set of variables (Delta_Ut represents the delta between target utilisation
        Delta_Ut = LpVariable.dicts(
            "DeltaTarget",
            [
                (b1, b2)
    ...
    for ma in MAlist:
        prob += (
            lpSum([MA_var[ma2] * G.BatchSize[ma2][week] for ma2 in MAlist])
            <= allQty + G.BatchSize[ma][week] - 1
        )
    ...
    print(
        (
            ...
            sum(MA_var[ma].varValue * G.BatchSize[ma][week] for ma in MAlist),
        )
    )
    allocation = {}
    for ma in MAlist:
