    ...
    while sufficient == False:
        ...
        if surplus == 0:
            ...
            if remUnits[currentMA] >= surplus:
                ...
                correctedQty = (
                    remainingUnits + G.BatchSize[currentMA][currentWeek] - surplus
                )
        ...
        for bottleneck in G.RouteDict[currentMA]:
            remainingCapacity[bottleneck] = (
                currentCapacity[bottleneck][currentWeek] - requiredCapacity[bottleneck]
            )
        ...
        if sufficient:
            ...
            for bottleneck in G.RouteDict[currentMA]:
                ...
                utilisation[bottleneck][currentWeek] = (
                    float(
                        G.Capacity[bottleneck][currentWeek]["OriginalCapacity"]
                        - currentCapacity[bottleneck][currentWeek]
                    )
                    / G.Capacity[bottleneck][currentWeek]["OriginalCapacity"]
                )
            Allocation.append(
                {"ma": currentMA, "units": correctedQty, "week": currentWeek}
            )
        ...
        # if the capacity available is not sufficient, the max allocable qty is derived
            ...
            for bottleneck in remainingCapacity:
                if (
                    ...
                    excessUnits = (
                        remainingCapacity[bottleneck]
                        / G.RouteDict[currentMA][bottleneck][currentWeek]
                    )
            ...
            for bottleneck in G.RouteDict[currentMA]:
                currentCapacity[bottleneck][currentWeek] -= (
                    allocableQty * G.RouteDict[currentMA][bottleneck][currentWeek]
                )

            Allocation.append(
                {"ma": currentMA, "units": allocableQty, "week": currentWeek}
            )
            if currentWeek > demandWeek:
