...
from .AllocManagement_Hybrid import (
...
def main(input, algorithmAttributes):
    ...
    for j in G.acoRange:
        for i in G.minRange[j]:
            ...
            for key in list(G.LateMeasures.keys()):
                if key == "lateness" or key == "earliness":
                    if len(G.LateMeasures[key]):
                        G.Summary[(G.ACO, G.minDeltaUt)][key] = mean(
                            G.LateMeasures[key]
                        )
                    else:
            ...
            G.Summary[(G.ACO, G.minDeltaUt)]["targetM"] = mean(
                absolute(array(targetUt))
            )
    ...
    listSummary = sorted(
        listSummary,
        key=itemgetter(
            "exUnits", "lateness", "targetM", "targetStd", "utilisation", "earliness"
        ),
    )
