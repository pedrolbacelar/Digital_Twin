...
process_time = 45    # advised to have no less than 5s of gap between two parts in the downstream conveyor to not have problems with pusher.
standard_deviation = 2
process_distribution = 'norm'
failure_probability = 0.000001 # percent
...
def on_message(client, userdata, msg):
    ...
    if msg.topic == "process_time":
        ...
        message = translate_message(msg)    #-- expected: {"macihne_id":"3","process_time":"10"}
        if message["machine_id"] == st_num: 
    ...
    while True:
        if system_status == "start":
            ...
            if queue_sensor.value() > 20 and station_status == "idle":
                pusher.run_forever(speed_sp=pusher_speed)
                payload_start = {"machine_id" : st_num, "status":"Started","part_id":"0","queue_id":"3"}
