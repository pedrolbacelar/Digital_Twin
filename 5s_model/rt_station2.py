...
st_num = "2"      #--- change st_num for each code depending on the machine_id
...
blocking_sensor_2.mode='COL-REFLECT'
btn = Button()
...
process_time = 9    # advised to have no less than 5s of gap between two parts in the downstream conveyor to not have problems with pusher.
...
def on_message(client, userdata, msg):
    ...
    if msg.topic == "RCT_server":
        ...
        if message["machine_id"] == st_num:    # we enter the policy advice into json dictionay for future use. You can send the part policy whenever you want.
            ...
            print("\033[32m#############################################################################")
            print("==== RCT_sever advice for station ",st_num," is : part id - ", message["part_id"], "dispatch to Queue ID - ", advice_list[message["part_id"]], "====")
            print("#############################################################################\033[0m")
    ...
    if msg.topic == "process_time":
        ...
        message = translate_message(msg)    #-- expected: {"macihne_id":"2","process_time":"10"}
        if message["machine_id"] == st_num: 
    ...
    while True:
        if system_status == "start":
            ...
            if queue_sensor.value() > 20 and station_status == "idle":
                pusher.run_forever(speed_sp=pusher_speed)
                payload_start = {"machine_id" : st_num, "status":"Started","part_id":"0","queue_id":"2"}
                ...
                pusher.stop(stop_action='coast')
                sleep(1)
                print("pusher retracted to allow part. Printing [system_status, station_status] = ",system_status, station_status, )
                pusher.run_forever(speed_sp = -pusher_speed)
                print("pusher is pushing the part inside. Printing [system_status, station_status] = ",system_status, station_status, )
                sleep(0.8)
                print("pusher about to stop pushing action. Printing [system_status, station_status] = ",system_status, station_status, )
                pusher.stop(stop_action='coast')
                print("pusher stopped pushing. Printing [system_status, station_status] = ",system_status, station_status, )
                station_status = "busy"
            ...
            if station_sensor.value() > 20 and station_status == "busy" and system_status == "start":
                ...
                while release:
                    ...
                    #--- functions during machine release    
                    if blocking_sensor_1.value() < 20 and blocking_sensor_2.value() < 20 and manual_failure == False:
