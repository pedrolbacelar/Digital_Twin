...
class Database():
    ...
    def write_event(self, table, timestamp, machine_id, activity_type, part_id):
        #--- Write the given evento into the the database
        
        with sqlite3.connect(self.database_path) as digital_model_DB: 
            digital_model_DB.execute(f"""
            INSERT INTO {table} (timestamp, machine_id, activity_type, part_id)
