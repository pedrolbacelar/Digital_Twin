...
class RandomNumberGenerator(object):
    ...
    def __init__(self, obj, distribution):
        ...
        for key in list(distribution.keys()):
            if key in [
                ...
                "Cauchy",
                "Triangular",
                "UniformSP",    # these are the distributions that use the SciPy parameters
                "NormalSP",
                "GammaSP",
                "ExpSP",
                "ErlangSP",
        ...
        # Parameters for the distributions using SciPy
        self.locSP = float(parameters.get("locSP", 0))      # get the location parameter
        self.scaleSP = float(parameters.get("scaleSP", 0))  # get the scale parameter
        self.shapeSP = float(parameters.get("shapeSP", 0))  # get the shape parameter
        self.a = float(parameters.get("aSP", 0))            # get the a parameter
        self.b = float(parameters.get("bSP", 0))            # get the b parameter

    def __repr__(self):
        return f'{self.distribution!r}'
    ...
    def generateNumber(self):
        ...
        # Distributions with SciPy parameters
        elif self.distributionType == "UniformSP":
            return stats.uniform.rvs(loc=self.locSP, scale=self.scaleSP)
        elif self.distributionType == "NormalSP":
            return stats.norm.rvs(loc=self.locSP, scale=self.scaleSP)
        elif self.distributionType == "GammaSP":
            return stats.gamma.rvs(self.shapeSP, loc=self.locSP, scale=self.scaleSP)
        elif self.distributionType == "ExpSP":
            return stats.expon.rvs(loc=self.locSP, scale=self.scaleSP)
        elif self.distributionType == "ErlangSP":
            return stats.erlang.rvs(self.shapeSP, loc=self.locSP, scale=self.scaleSP)
        elif self.distributionType == "LognormSP":
            return stats.lognorm.rvs(self.shapeSP, loc=self.locSP, scale=self.scaleSP)
        elif self.distributionType == "TriangularSP":
            return stats.triang.rvs(self.shapeSP, loc=self.locSP, scale=self.scaleSP)
        elif self.distributionType == "BetaSP":
            return stats.beta.rvs(self.a, self.b, loc=self.locSP, scale=self.scaleSP)
        elif self.distributionType == "ParetoSP":
            return stats.pareto.rvs(self.shapeSP, loc=self.locSP, scale=self.scaleSP)
        elif self.distributionType == "LogisticSP":
            return stats.logistic.rvs(loc=self.locSP, scale=self.scaleSP)
        elif self.distributionType == "RayleighSP":
            return stats.rayleigh.rvs(loc=self.locSP, scale=self.scaleSP)
        else:
            raise ValueError(
                "Unknown distribution %r used in %s %s"
