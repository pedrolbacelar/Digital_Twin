    ...
    for priority in G.priorityList["order"]:
        ...
        for week in G.WeekList:
            ...
            if week in G.sortedOrders["order"][priority]:
                ...
                ACOresults = Allocation_ACO(
                    week, G.sortedOrders["order"][priority][week], "order", ACOresults
                )
    ...
    for priority in G.priorityList["forecast"]:
        ...
        for week in G.WeekList:
            ...
            if week in G.sortedOrders["forecast"][priority]:
                ...
                AllocationRoutine_Forecast(
                    week, G.sortedOrders["forecast"][priority][week], "forecast"
                )
        ...
        for week in G.WeekList:
            ...
            if week in G.sortedOrders["order"][priority]:
                ...
                if G.ACO:
                    if G.ACOdefault:
                        G.popSize = int(
                            0.75 * len(G.sortedOrders["order"][priority][week])
                            - 0.75 * len(G.sortedOrders["order"][priority][week]) % 2
                        )
                        G.noGen = 20 * G.popSize
                    ACOresults, anting = Allocation_ACO(
                        week,
                    ...
                    if bestAnt != None:
                        ...
                        AllocationRoutine_Final(
                            week, G.sortedOrders["order"][priority][week], "order", 0
                        )
        ...
        for week in G.WeekList:
            ...
            if week in G.sortedOrders["forecast"][priority]:
                ...
                if G.GA:
                    if G.GAdefault:
                        G.popSizeGA = int(
                            0.75 * len(G.sortedOrders["forecast"][priority][week])
                            - 0.75 * len(G.sortedOrders["forecast"][priority][week]) % 2
                        )
                ...
                # if GA is not require perform allocation with internal LP optimisation
                    ...
                    AllocationRoutine_Forecast(
                        week, orderList, "forecast", {"seq": orderIDlist}
                    )
