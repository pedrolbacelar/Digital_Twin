...
def ranking(candidates, elg):
    ...
    if (
        fittestLateness[0]["excess"]
        == fittestLateness[len(fittestLateness) - 1]["excess"]
        and fittestLateness[0]["lateness"]
        == fittestLateness[len(fittestLateness) - 1]["lateness"]
        and fittestLateness[0]["earliness"]
        == fittestLateness[len(fittestLateness) - 1]["earliness"]
    ):
        ...
        if numFit < elg:
            ...
            for i in range(len(fittestUtilisation)):
                ...
                ultRanking[fittestUtilisation[i]["orderSequence"]]["avg"] = mean(
                    [
                        ultRanking[fittestUtilisation[i]["orderSequence"]][
                            "utilisation"
                        ],
...
def Allocation_ACO(initialWeek, itemList, itemType, ACOresults):
    ...
    for gen in range(G.noGen):
        ...
        for a in ants:
            ACOresults.append(
                (initialWeek, gen, rep, a["ant"]["antID"], "survived", "", "", "", "")
            )
    ...
    ACOresults.append(
        (initialWeek, gen, rep, ant["ant"]["antID"], "selected", "", "", "", "")
    )
