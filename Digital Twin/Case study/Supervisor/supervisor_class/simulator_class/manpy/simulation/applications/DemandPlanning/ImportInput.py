...
def ImportInput(input, algorithmAttributes):
    ...
    for row in range(2, rows, 4):
        ...
        G.Capacity[
            ...
            {}
        )  # {'OriginalCapacity':{}, 'RemainingCapacity':{}, 'minUtilisation':{}, 'targetUtilisation':{}}
        G.CurrentCapacityDictOrig[bn] = {}

        for week in range(w, sh.ncols):
            G.Capacity[bn][Weeks[week]] = {
                "OriginalCapacity": withoutFormat(row, week, sh, 1),
                "RemainingCapacity": withoutFormat(row + 1, week, sh, 1),
                "minUtOrig": withoutFormat(row + 2, week, sh, 0),
                "targetUtOrig": withoutFormat(row + 3, week, sh, 0),
            }
            if G.Capacity[bn][Weeks[week]]["minUtOrig"] == "":
                G.Capacity[bn][Weeks[week]]["minUtilisation"] = 0
            else:
                G.Capacity[bn][Weeks[week]]["minUtilisation"] = G.Capacity[bn][
                    Weeks[week]
                ]["minUtOrig"]
            ...
            if G.Capacity[bn][Weeks[week]]["targetUtOrig"] == "":
                ...
                G.Capacity[bn][Weeks[week]]["targetUtilisation"] = G.Capacity[bn][
                    Weeks[week]
                ]["targetUtOrig"]
        ...
        for week in range(4, sh.ncols):
            ...
            if qty:
                ...
                for ma in range(subRow):
                    G.orders[orderID]["suggestedMA"][maSuggested[ma]] = withoutFormat(
                        row + ma, week, sh, 1
                    )
                ...
                G.sortedOrders["forecast"][priority][Weeks[week]].append(
                    G.orders[orderID]
                )
                oID += 1
                orderID = "Forecast_" + str(oID)
